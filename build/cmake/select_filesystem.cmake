...
elseif(NANA_CMAKE_BOOST_FILESYSTEM_FORCE)
    ...
    if (TEST_FS_LIB)
        ...
        cmake_print_variables(CMAKE_REQUIRED_FLAGS)
        ...
        cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            ...
            check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXXBuiltIn_FS_BuiltIn)
            cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXXBuiltIn_FS_BuiltIn)
            ...
            check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXXBuiltIn_FS_stdcppfs)
            cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXXBuiltIn_FS_stdcppfs)
            ...
            check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXXBuiltIn_FS_cppfs)
            cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXXBuiltIn_FS_cppfs)
            ...
            set (CMAKE_REQUIRED_LIBRARIES ${CMAKE_REQUIRED_LIBRARIES_ORIGINAL})
            check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std__cpp17_FS_BuiltIn)
            cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXX_std__cpp17_FS_BuiltIn)
            ...
            set (CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS_ORIGINAL} -std=c++17")
            set (CMAKE_REQUIRED_LIBRARIES "${CMAKE_REQUIRED_LIBRARIES_ORIGINAL} stdc++fs")
            check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std__cpp17_FS_stdcppfs)
            cmake_print_variables(CMAKE_REQUIRED_FLAGS)
            cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXX_std__cpp17_FS_stdcppfs)
           ...
           set (CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS_ORIGINAL} -std=c++17")
           set (CMAKE_REQUIRED_LIBRARIES "${CMAKE_REQUIRED_LIBRARIES_ORIGINAL}  c++fs")
           check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std__cpp17_FS_cppfs)
           cmake_print_variables(CMAKE_REQUIRED_FLAGS)
           cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
        if (CXX_std__cpp17_FS_cppfs)
           ...
           set (CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS_ORIGINAL} /std:c++17")
           set (CMAKE_REQUIRED_LIBRARIES ${CMAKE_REQUIRED_LIBRARIES_ORIGINAL})
           check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std_cpp17_FS_BuiltIn)
           cmake_print_variables(CMAKE_REQUIRED_FLAGS)
           cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
       if (CXX_std_cpp17_FS_BuiltIn)
           ...
           set (CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS_ORIGINAL} /std:c++17")
           set (CMAKE_REQUIRED_LIBRARIES "${CMAKE_REQUIRED_LIBRARIES_ORIGINAL} stdc++fs")
           check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std_cpp17_FS_stdcppfs)
           cmake_print_variables(CMAKE_REQUIRED_FLAGS)
           cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
       if (CXX_std_cpp17_FS_stdcppfs)
           ...
           set (CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS_ORIGINAL} /std:c++17")
           set (CMAKE_REQUIRED_LIBRARIES "${CMAKE_REQUIRED_LIBRARIES_ORIGINAL}  c++fs")
           check_cxx_source_compiles ("${CXXSTD_TEST_SOURCE}" CXX_std_cpp17_FS_cppfs)
           cmake_print_variables(CMAKE_REQUIRED_FLAGS)
           cmake_print_variables(CMAKE_REQUIRED_LIBRARIES)
